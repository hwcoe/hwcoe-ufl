...
class UFL_2015_Breaker_Cards extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 
			...
			'description' => __('Background Image with content cards.', 'hwcoe-ufl'),
			'customize_selective_refresh' => true,
		);
		$control_ops = array();
		parent::__construct( 'ufl-breaker-cards', __('UFL Breaker with Cards', 'hwcoe-ufl'), $widget_ops, $control_ops );
	}
	...
	public function form( $instance ) {
        ...
        <p>
		<label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Post Category', 'hwcoe-ufl'); ?></label>
		<select class="widefat" id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" value="<?php echo $category; ?>" style="width:100%">
			<option value="" <?php selected($category); ?>><?php _e('Select Category', 'hwcoe-ufl'); ?></option>
